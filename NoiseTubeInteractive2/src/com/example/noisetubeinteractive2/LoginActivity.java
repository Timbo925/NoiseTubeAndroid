...
public class LoginActivity extends Activity {
	...
	public void attemptLogin() {
		...
		if (TextUtils.isEmpty(mUserName)) {
			...
			mUserNameView.setError("Username needs be longer then 5 character");
			focusView = mUserNameView;
			cancel = true;
		}
	...
	public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			if (jsonResponse.hasErrors()) {
				Log.d("LoginActivity", "Login Failed with jsonResponse: " + jsonResponse);
				return false;
			} else {
				try {
					JSONObject json = new JSONObject(jsonResponse.getMessage());
					sessionId = json.getString("sessionId");
					
					SessionStorage sessionStorage = new SessionStorage(getApplicationContext());
					sessionStorage.setSession(sessionId);
					Log.d("LoginActivity", "New sessionId: " + sessionId);
					return true;
